...
/// I/O types, and a module augmentation for `@flux/sdk`.
///
...
/// Map a Flux column type to the appropriate TypeScript primitive.
...
/// `@flux/functions`) to a TypeScript type string.
...
fn generate_sdk(db_schema: &Value, functions: &[Value], schema_hash: &str) -> String {
    ...
    // ── File header ───────────────────────────────────────────────────────
    out.push_str("// Auto-generated by Flux SDK Generator — do not edit manually.\n");
    ...
    // ── Module augmentation ───────────────────────────────────────────────
    // Augments the `@flux/sdk` module so the dynamically-proxied
    // `flux.db.*` and `flux.functions.*` objects are fully typed.
    out.push_str("declare module \"@flux/sdk\" {\n");
    ...
    for name in &table_names {
        let iface = to_pascal(name);
        out.push_str(&format!(
            "    {}: import(\"@flux/sdk\").TableClient<{}, Insert{}, Update{}>;\n",
            name, iface, iface, iface
        ));
    }
    out.push_str("  }\n\n");

    // FluxFunctions augmentation
    ...
    out.push_str("export { createClient } from \"@flux/sdk\";\n");
