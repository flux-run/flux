...
pub async fn list_api_keys(
    ...
    let rows = sqlx::query_as::<_, ApiKeyRow>(
        "SELECT id, name, key_prefix, created_at, last_used_at \
         FROM flux.api_keys WHERE revoked_at IS NULL \
...
pub async fn create_api_key(
    ...
    let row = sqlx::query_as::<_, ApiKeyRow>(
        "INSERT INTO flux.api_keys (id, name, key_hash, key_prefix) \
         VALUES ($1, $2, $3, $4) \
...
pub async fn delete_api_key(
    ...
    sqlx::query("UPDATE flux.api_keys SET revoked_at = now() WHERE id = $1")
        .bind(id)
        .execute(&state.pool)
        .await
        .map_err(db_err)?;
