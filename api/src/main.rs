...
async fn init_local_mode(pool: &sqlx::PgPool) -> Result<(Uuid, Uuid), sqlx::Error> {
    ...
    // Tenant may have a nullable owner_id FK — insert with ON CONFLICT DO NOTHING
    sqlx::query(
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let (local_tenant_id, local_project_id) = init_local_mode(&pool).await?;
