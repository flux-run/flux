...
<pre><code><span class="shell-prompt">$</span> curl -fsSL https://fluxbase.co/install | bash
  ...
  <span class="str">✔</span>  flux 1.0.0 installed to /usr/local/bin/flux

<span class="shell-prompt">$</span> flux login
  <span class="str">✔</span>  Logged in as alice@example.com</code></pre>
...
<pre><code><span class="kw">export default</span> <span class="fn">defineFunction</span>({
  ...
  handler: <span class="kw">async</span> ({ input, ctx }) =&gt; {
    ...
    <span class="kw">await</span> ctx.workflow.<span class="fn">run</span>(<span class="str">"send_welcome"</span>, { userId: user.id });
...
<pre><code><span class="shell-prompt">$</span> flux deploy
  ...
  <span class="str">✔</span>  create_user   → gw.fluxbase.co/create_user
