...
export function createConcurrencySuite(): TestHarness {
  ...
  suite.test("Microtask vs macrotask ordering", async () => {
    ...
    assert(
      order.indexOf("microtask1") < order.indexOf("macrotask"),
      "Microtasks should run before macrotasks"
    );
  });
  ...
  suite.test("Concurrent array modifications", () => {
    const arrays = Array.from({ length: 3 }, () => [] as number[]);
