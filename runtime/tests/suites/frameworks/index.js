...
export function createFrameworkSuite() {
    ...
    suite.test("Header handling", () => {
        ...
        assertEquals(headers.get("authorization"), "Bearer token123", "Authorization header should work");
    ...
    suite.test("Rate limiting", () => {
        const requests = {};
        const checkRateLimit = (clientId, limit, windowMs) => {
            const now = Date.now();
