...
pub async fn validate_service_token(url: &str, service: &str, token: &str) -> Result<String> {
    ...
    let mut request = Request::new(pb::ValidateServiceTokenRequest {
        service_name: service.to_string(),
    });
    request.metadata_mut().insert(
        "x-service-token",
        MetadataValue::try_from(token).context("service token contains invalid metadata characters")?,
    );
