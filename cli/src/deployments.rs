...
pub async fn execute(_args: DeploymentsArgs) -> Result<()> {
    ...
    let project_dir = if load_project_config(&cwd).is_ok() {
        ...
        found.ok_or_else(|| {
            anyhow::anyhow!(
                "No flux.json found in current or parent directories. Are you in a Flux project?"
            )
        })?
    };
    ...
    let history: shared::project::BuildHistory =
        serde_json::from_str(&source).context("failed to parse deployments.json")?;
    ...
    if history.deployments.is_empty() {
        ...
        for dep in history.deployments.iter().rev() {
            ...
            println!(
                "  \x1b[32m{}\x1b[0m  {:^20}  {}",
                short_sha, dep.timestamp, dep.entry
            );
        }
    }
